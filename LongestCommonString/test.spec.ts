...
import { longestCommonSubsequence as longestCommonSubsequenceJZ } from './solution.jz'
...
test('最长公共子序列', () => {
  expect(longestCommonSubsequenceJZ('1234567', '1357')).toBe(4);
  expect(longestCommonSubsequenceJZ('abc', 'abc')).toBe(3);
  expect(longestCommonSubsequenceJZ('123', '456')).toBe(0);
  expect(longestCommonSubsequenceJZ('asd', 'dfg')).toBe(1);
})
